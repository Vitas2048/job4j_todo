...
public class SimpleTaskRepository implements TaskRepository {
    ...
    public List<Task> filterBy(boolean condition, int userId) {
        return crudRepository.query("from Task f left JOIN FETCH f.priority left join fetch f.categories where done = :fDone and user_id=:fUserId",
                Task.class, Map.of("fDone", condition, "fUserId", userId));
    }

    @Override
    public List<Task> findAllByUser(int userId) {
        return crudRepository.query("from Task f left JOIN FETCH f.priority left join fetch f.categories where user_id=:fUserId",
